...
const getDashboardScrollStorageKey = (audience) => `dashboard-feed-scroll-position:${audience}`;
...
export default function DashboardClient() {
  ...
  const hasRestoredFeedScrollRef = useRef(false);
  ...
  useEffect(() => {
    ...
    const savedFeedScroll = Number(window.sessionStorage.getItem(getDashboardScrollStorageKey(selectedAudience)) || 0);
    restoreFeedScrollRef.current = Number.isFinite(savedFeedScroll) ? savedFeedScroll : 0;
    hasRestoredFeedScrollRef.current = false;
  }, [selectedAudience]);
  ...
  const persistFeedScroll = (scrollTop) => {
    ...
    window.sessionStorage.setItem(getDashboardScrollStorageKey(selectedAudience), String(safeScrollTop));
  };
  ...
  useEffect(() => {
    ...
    if (hasRestoredFeedScrollRef.current && !restorePostId) return;
    ...
    const restoreFeedPosition = () => {
      ...
      hasRestoredFeedScrollRef.current = true;
