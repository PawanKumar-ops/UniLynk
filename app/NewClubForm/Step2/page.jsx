...
export default function Step2() {
  ...
  const [activities, setActivities] = useState([
    { title: "", date: "", description: "", participants: "" },
  ]);
  ...
  const handleNext = () => {
    localStorage.setItem("step2Data", JSON.stringify(activities));
    router.push("/NewClubForm/Step3");
  };

  const handleBack = () => {
    router.push("/NewClubForm");
  };
    ...
    <div className="step-container">
      ...
      <main className="form-content">
        <div className="form-header">
          <h1 className="form-title">Past Activities</h1>
          <p className="form-description">Showcase your club's previous events and achievements</p>
        </div>
          ...
          {activities.map((activity, index) => (
            <div key={index} className="activity-card">
              <div className="activity-header">
                <h3 className="activity-title">Activity {index + 1}</h3>
                {activities.length > 1 && (
                  <button
                    type="button"
