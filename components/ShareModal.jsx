...
const MOCK_CONTACTS = [
  ...
  { id: 3, name: "James Wilson", initials: "JW" },
  { id: 4, name: "Sarah Chen", initials: "SC" },
  { id: 5, name: "David Kim", initials: "DK" },
  { id: 6, name: "Emma Davis", initials: "ED" },
  { id: 7, name: "Michael Brown", initials: "MB" },
  { id: 8, name: "Olivia Taylor", initials: "OT" },
];
    ...
    >
      <div className={`share-modal${closing ? " closing" : " open"}`} role="dialog" aria-label="Share Post">
        ...
        <div className="share-modal-section">
          <p className="share-modal-section-title">Share to</p>
          <div className="share-external-grid">
        ...
        <div className="share-modal-section">
          <p className="share-modal-section-title">Send in chat</p>
          <input
            className="share-chat-search"
            type="text"
          ...
          <div className="share-chat-list">
            {filteredContacts.map((contact) => (
              <div key={contact.id} className="share-chat-item">
                <div className="share-chat-avatar">{contact.initials}</div>
                <span className="share-chat-name">{contact.name}</span>
                <button
                  className={`share-chat-send${sentIds.has(contact.id) ? " sent" : ""}`}
                  onClick={() => handleSend(contact.id)}
                  disabled={sentIds.has(contact.id)}
                >
                  {sentIds.has(contact.id) ? "Sent" : "Send"}
                </button>
              </div>
            ))}
            {filteredContacts.length === 0 && (
              <p style={{ textAlign: "center", color: "#999", fontSize: 14, padding: 20 }}>No contacts found</p>
            )}
          </div>
        </div>
      </div>
