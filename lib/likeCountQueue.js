...
const flushQueue = async () => {
  ...
  try {
    ...
    if (operations.length) {
      ...
      try {
        const { updatePostTrendingScore } = await import("./feedRanking");
        for (const [postId] of pending) {
          updatePostTrendingScore(postId).catch(err =>
            console.error(`Error updating trending score for ${postId} on flush:`, err)
          );
        }
      } catch (err) {
        console.error("Failed to import/run trending score updates:", err);
      }
    }
  } catch (error) {
