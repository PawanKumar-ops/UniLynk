...
export function hashOtp(email, otp, purpose) {
  if (!(process.env.AUTH_SECRET ?? process.env.NEXTAUTH_SECRET)) throw new Error("AUTH_SECRET (or NEXTAUTH_SECRET during migration) is required");
  return crypto
...
export function signVerificationToken(email, purpose) {
  if (!(process.env.AUTH_SECRET ?? process.env.NEXTAUTH_SECRET)) throw new Error("AUTH_SECRET (or NEXTAUTH_SECRET during migration) is required");
  return jwt.sign({ email: normalizeEmail(email), purpose, type: "otp_verified" }, (process.env.AUTH_SECRET ?? process.env.NEXTAUTH_SECRET), {
    expiresIn: "10m",
  });
}
...
export function verifyVerificationToken(token, email, purpose) {
  if (!(process.env.AUTH_SECRET ?? process.env.NEXTAUTH_SECRET)) throw new Error("AUTH_SECRET (or NEXTAUTH_SECRET during migration) is required");
  try {
    const payload = jwt.verify(token, (process.env.AUTH_SECRET ?? process.env.NEXTAUTH_SECRET));
